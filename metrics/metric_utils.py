...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, **stats_kwargs):
    ...
    #print(item_subset)
    for images, images_NIR, _, _labels in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        #print(masks.shape)
        # TODO Here? Metrics? 
        #print("...")
        #print(images.shape)
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, jit=False, **stats_kwargs):
    ...
    def run_generator(z, c, arcface_feat):
        img, img_NIR = G(z=z, c=c, arcface_feat=arcface_feat, **opts.G_kwargs)
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            arcface_feat = torch.empty([batch_gen, G.arcface_dim], device=opts.device) #TODO 
            
            img, img_NIR = run_generator(z, c, arcface_feat)
