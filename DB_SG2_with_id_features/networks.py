...
def modulated_conv2d(
    ...
    #print("DEMODULATE:", demodulate)
    #print("FUSED_MOD_CONV:", fused_modconv)
    ...
    if return_intermediate_xs: # TODO make faster
            return x, None     
    return x
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        if self.return_xs_after_AdaIN: 
            x, x0 = modulated_conv2d(x=x, weight=self.weight, styles=styles, noise=noise, up=self.up,
                                ...
                                # TODO tle fused_modconv = False
                                # pa pol porihtat da mas x x0 izhode 
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == 'skip':
            ...
            self.toseg = ToSEGLayer(out_channels, 1, w_dim=w_dim, conv_clamp=conv_clamp, channels_last=self.channels_last)
    ...
    def forward(self, x, img, NIR_img, ws, force_fp32=False, fused_modconv=None, **layer_kwargs):
        ...
        if self.is_last or self.architecture == 'skip':
            ...
            y_NIR = self.toseg(x, tmp_w, fused_modconv=fused_modconv)
            y_NIR = y_NIR.to(dtype=torch.float32, memory_format=torch.contiguous_format)
            
            if img is not None: 
                img = img.add_(y)
                NIR_img = NIR_img.add(y_NIR)
            else:
                img = y
                NIR_img = y_NIR
...
class DiscriminatorBlock(torch.nn.Module):
    ...
    def forward(self, x, img, force_fp32=False):
        ...
        if self.in_channels == 0 or self.architecture == 'skip':
            ...
            #print("BEFORE FROM RGB:", img.shape)
            y = self.fromrgb(img)
            #print("After from rgb:", y.shape)
