...
def BACK_setup_snapshot_image_grid(training_set, random_seed=0):
    rnd = np.random.RandomState(random_seed)
    gw = np.clip(3840 // training_set.image_shape[2], 7, 32)
    gh = np.clip(2160 // training_set.image_shape[1], 4, 32)
    ...
    # TODO use id from file_name instead of name ... 
    # TODO ... label differently ... maybe include in dataset the actual class 
    
    else:
        tmp_dict_id_to_label = dict()

        # Group training samples by label.
        label_groups = dict() # label => [idx, ...]
        for idx in range(len(training_set)):
            #raw_idx = training_set._raw_idx[idx]
            label = training_set.get_details(idx).f_id  # f_id is the file_id from [0 ... 104]
            #print(label)
            feat = tuple(training_set.get_details(idx).raw_label.flat[::-1])
            if label not in label_groups:
                label_groups[label] = []
            label_groups[label].append(idx)
            
            if label not in tmp_dict_id_to_label:
                tmp_dict_id_to_label[label] = feat 
            else: 
                if tmp_dict_id_to_label[label] != feat: 
                    print("Something is wrong here, features do not matches")
                    #print(feat)
                    #print(tmp_dict_id_to_label[label])
                    return 
        # Reorder.
        label_order = sorted(label_groups.keys())
        for label in label_order:
...
def training_loop(
    ...
    # loss_kwargs['NIR_loss_weight'] = loss_kwargs.nir_loss_weight
